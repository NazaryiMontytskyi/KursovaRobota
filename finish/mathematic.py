...
class SolvingSystem:
    ...
    def check_borders_of_values(self):
        to_check = [self.a, self.b, self.c, self.d, self.f, self.g]
        result:bool
        for i in range(0, len(to_check)):
            if abs(to_check[i]) < 30:
                result = True
            else:
                result = False
                break
    ...
    def solve_system_of_equations(self):
        ...
        try:
            ...
            if self.win.choose_equation.currentIndex() == 2:
                itterations_text += "Тригонометрична система\n"
                itterations_text += f"{self.a} * tg^3(x1) + {self.b} * cos({self.c} * x2) = 0\n"
                itterations_text += f"{self.d} * sin({self.f} * x1) - {self.g} * tg(x2) = 0\n"
            ...
            while self.it <= 1000:
                ...
                itterations_text += f"Ітерація №{self.it}\n"
                ...
                if abs(norm_of_vector(curr) - norm_of_vector(prev_var)) <= self.e:
                    ...
                    break
            if self.it > 1000:
                self.win.message_class.system_endless_loop()
            else:
                if self.index_eq == 1:
                    if int(self.a * (self.x1 ** 3) - self.b * (self.x2 ** 3) + self.c) == 0 and int(
                        ...
                        self.win.message_class.system_no_solutions()
                if self.index_eq == 2:
            ...
            self.win.message_class.system_value_error()
                ...
                if self.d == 0 and self.f == 0:
                    ...
                    if self.index_eq == 1:
                        ...
                        self.win.iterations_list.label.setText(
                            f"Система рівнянь розв'язана окремо від ітераційного процесу\nx₁ = {self.x1}\nx₂ = {self.x2}\n")
                        self.need_itteration_procces = False
                        self.is_solved = True
    ...
    def draw_graph(self):
        ...
        if self.index_eq == 1:
            if self.need_itteration_procces is True:
                ...
                second_x_1 = np.linspace(-100, -(self.d * (self.x1 ** 2) - self.f * (self.x2 ** 2) + self.g),
                                         max_size)
                second_y_1 = np.sqrt(np.where(((self.d * (second_x_1 ** 2) + self.g) / self.f) >= 0, ((self.d * (second_x_1 ** 2) + self.g) / self.f), np.nan))

                second_x_2 = np.linspace((self.d * (self.x1 ** 2) - self.f * (self.x2 ** 2) + self.g), 100,
                                         max_size)

                second_y_2 = np.sqrt(np.where(((self.d * (second_x_2 ** 2) + self.g) / self.f) >= 0, ((self.d * (second_x_2 ** 2) + self.g) / self.f), np.nan))

                second_x_3 = np.linspace(-100, -(self.d * (self.x1 ** 2) - self.f * (self.x2 ** 2) + self.g),
                                         max_size)
                second_y_3 = -np.sqrt(np.where(((self.d * (second_x_3 ** 2) + self.g) / self.f) >= 0, ((self.d * (second_x_3 ** 2) + self.g) / self.f), np.nan))

                second_x_4 = np.linspace((self.d * (self.x1 ** 2) - self.f * (self.x2 ** 2) + self.g), 100,
                                         max_size)
                second_y_4 = -np.sqrt(np.where(((self.d * (second_x_4 ** 2) + self.g) / self.f) >= 0, ((self.d * (second_x_4 ** 2) + self.g) / self.f), np.nan))
        ...
        solve_point = PlotDataItem()
        solve_point.setData([self.x1], [self.x2], symbol='o', pen='g', brush='b', size=1)
